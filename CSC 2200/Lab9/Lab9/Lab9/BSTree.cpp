...

{
	...
	{
		ret = retrieveHelper(p->left, searchKey, searchDataItem);
	}
	else if (searchKey > p->dataItem.getKey())
	{
		ret = retrieveHelper(p->right, searchKey, searchDataItem);
	}
	else
	{
...

{
	...
	{
		Lft = getHeightHelper(p->left) + 1;   
		Rght = getHeightHelper(p->right) + 1; 
		if (Lft >= Rght)                  
			result = Lft;
		else
			result = Rght;
