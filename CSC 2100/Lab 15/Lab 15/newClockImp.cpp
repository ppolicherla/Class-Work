...
clockType& operator++(clockType& clock, int x)
{
	clockType temp = clock;
...
bool operator<(const clockType& sClock, const clockType& otherClock) 
{
	return ((sClock.hr < otherClock.hr) ||
		(sClock.hr == otherClock.hr && sClock.min < otherClock.min) ||
		(sClock.hr == otherClock.hr && sClock.min == otherClock.min &&
		sClock.sec < otherClock.sec));
}
